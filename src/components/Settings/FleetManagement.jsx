...
import { Truck, Search, Plus, Activity, History, Edit, Trash2, MoreVertical } from "lucide-react";
...
export default function FleetManagement() {
  ...
  const fetchVehicles = async () => {
    setLoading(true);
    try {
      const data = await api.getCollection("vehicles");
      setVehicles(data);
    } catch (err) {
  ...
  return (
    <div style={{ padding: "32px 40px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, marginBottom: 32 }}>
        <FleetStat label="Total Vehicles" value={vehicles.length} icon={Truck} color="#2563eb" />
        <FleetStat label="Active on Road" value={vehicles.filter(v => v.status === "On Trip").length} icon={Activity} color="#16a34a" />
        <FleetStat label="Under Maintenance" value={vehicles.filter(v => v.status === "Maintenance").length} icon={History} color="#f59e0b" />
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, overflow: "hidden" }}>
        ...
        {loading ? (
          <div style={{ padding: 40, textAlign: "center", color: "#64748b" }}>Loading fleet...</div>
        ) : (
